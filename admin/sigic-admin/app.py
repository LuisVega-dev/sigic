...
def index():
    ...
    try:
        ...
        return render_template('index.html', usuarios=[], message=f"Error al obtener datos: {e}", message_type="danger")
...
@app.route('/add_admin/<token>', methods=['GET', 'POST'])
@login_required  # Asegura que solo los administradores puedan acceder
def add_admin(token):
    """
...
@app.route('/remove_admin/<token>', methods=['GET', 'POST'])
@login_required  # Asegura que solo los administradores puedan acceder
...
def admins():
    ...
    try:
        ...
        return render_template('admin_table.html', admins=admins)
    except Exception as e:
        flash(f"Error al obtener administradores: {e}", 'error')
        return render_template('admin_table.html', admins=[], message=f"Error al obtener administradores: {e}", message_type="error")
...
@app.route('/edit_profile/<token>', methods=['GET', 'POST'])
@login_required  # Asegura que solo los administradores puedan acceder
def edit_profile(token):
    ...
    try:
        if request.method == 'POST':
            ...
            cur.close()
            conn.close()
            return render_template('edit_profile.html', user_admin=admin)
    except Exception as e:
...
@app.template_filter('encrypt_id')
